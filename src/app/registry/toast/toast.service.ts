...
/**
 * Queues toast messages for `Toaster` to render (one `Toaster` mounted once, typically in the
 * app shell). Each entry is drawn with the existing `Notification` component in `stacked` mode —
 * this service only adds the queueing/auto-dismiss/stacking layer `Notification` doesn't have on
 * its own (it's a single controlled toast, not a queue).
 */
